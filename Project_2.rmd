...
###Model
```{r}
hotel_ann <- neuralnet(formula = is_canceled ~ .,data = hotel_norm_train, hidden = 3)
```

###Prediction
```{r}
ann_predictions <- predict(hotel_ann, hotel_norm, type="response")
ann_predictions <- as.factor(ifelse(ann_predictions>0.5,1,0))

write.csv(ann_predictions, "hotel_ann_pred.csv")
```
...
### Model
```{r}
svm <- ksvm(is_canceled ~ ., data = hotel, kernel = "rbfdot")
...
###Prediction
```{r}
c50_hotel_pred <- predict(hotel_decision_model, hotel_test)
write.csv(c50_hotel_pred, "decision_tree_pred.csv")
```

###Confusion Matrix
```{r}
c50_matrix <- confusionMatrix(c50_hotel_pred, as.factor(hotel_test$is_canceled))
c50_matrix
```
