...
### Predictions
```{r}
glm_prediction <- predict(glm, glm_test, type="response")
...
```{r}
knn <- knn(train = knn_train, test = knn_test,
...
c50_matrix <- confusionMatrix(as.factor(c50_hotel_pred$x), as.factor(actuals$x))
c50_matrix
```
...
glm_matrix <- confusionMatrix(as.factor(glm_predictions$x), as.factor(actuals$x))
glm_matrix

knn_matrix <- confusionMatrix(as.factor(knn_predictions$x), as.factor(actuals$x))
knn_matrix

ann_matrix <- confusionMatrix(as.factor(ann_predictions$x), as.factor(actuals$x))
ann_matrix

svm_matrix <- confusionMatrix(as.factor(svm_predictions$x), as.factor(actuals$x))
svm_matrix

c50_matrix <- confusionMatrix(as.factor(c50_predictions$x), as.factor(actuals$x))
c50_matrix
...
6034 predicted cancel
38% cancelation rate
daily average 42 people
overbook 16 around people everyday with 85% confidence
we can overbook a cerntain aomunt with 85% confidence
15866
